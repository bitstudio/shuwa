...
function getOffsetPoint(y: any, x: any, keypoint: any, offsetsBuffer: any) {
    return {
...
function getOffsetVectors(heatMapCoordsBuffer: any, offsetsBuffer: any) {
    const result = [];
...
function getOffsetPoints(
    heatMapCoordsBuffer: any,
    outputStride: number,
    offsetsBuffer: any,
) {
    return tf.tidy(() => {
        const offsetVectors = getOffsetVectors(
            heatMapCoordsBuffer,
            offsetsBuffer,
        );
...
function getPointsConfidence(heatmapScores: any, heatMapCoords: any) {
...
export const decodePose = async (
    heatmapScores: tf.Tensor2D,
    offsets: tf.Tensor2D,
    outputStride: number,
) => {
    let totalScore = 0.0;
...
export const local2GlobalKeypoints = (
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            // center at 0.
            const localKeypointsNorm = localKeypointsTensorMul.sub(
                globalBoxSize / 2,
            );
            // rotate keypoints.
...
export function getRadians(
    ...
    return new Promise((resolve) => {
        ...
        resolve(
            radians -
                2 * Math.PI * Math.floor((radians + Math.PI) / (2 * Math.PI)),
        );
    });
}

export function getDistance(pointA: number[], pointB: number[]) {
    return new Promise((resolve) => {
        resolve(
            Math.sqrt(
